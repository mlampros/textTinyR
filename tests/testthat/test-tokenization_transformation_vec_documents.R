...
testthat::test_that("the function returns a vector of words if string split is TRUE and stemmer is ngram_sequential", {
  ...
  tmp =  tokenize_transform_vec_docs(object = text_rows, as_token = TRUE, to_lower = TRUE, to_upper = FALSE, utf_locale = "", remove_char = "",
                           ...
                           split_separator = " \r\n\t.,;:()?!//", remove_stopwords = TRUE, language = "english", min_num_char = 1, max_num_char = Inf, stemmer = 'ngram_sequential', min_n_gram = 1,
                           
                           max_n_gram = 1, skip_n_gram = 1, skip_distance = 0, n_gram_delimiter = " ", concat_delimiter = NULL, path_2folder = "", stemmer_ngram = 4, stemmer_gamma = 0.0, 
                           
                           stemmer_truncate = 3, stemmer_batches = 1, threads = 1, vocabulary_path_file = NULL, verbose = FALSE)

  res_tes = names(tmp) == 'token'  && length(tmp$token) == 5 && is.list(tmp$token) && sum(unlist(lapply(tmp$token, function(x) is.character(x)))) == 5 && class(tmp) == "tokenization and transformation" && tmp$token[[2]][1] == 'planet'
...
testthat::test_that("the function returns a vector of words if string split is TRUE and stemmer is ngram_overlap", {
  ...
  res_tes = names(tmp) == 'token'  && length(tmp$token) == 5 && is.list(tmp$token) && sum(unlist(lapply(tmp$token, function(x) is.character(x)))) == 5 && class(tmp) == "tokenization and transformation" && tmp$token[[4]][1] == '_at_'
...
testthat::test_that("the function returns a vector of words if string split is TRUE and stemmer is ngram_sequential, AND as_token = FALSE", {
  ...
  res_tes = names(tmp) == 'token'  && length(tmp$token) == 5 && class(tmp) == "tokenization and transformation" && substring(tmp$token[2], 1, 4) == 'plan'
...
testthat::test_that("the function returns a vector of words if string split is TRUE and stemmer is ngram_overlap, AND as_token = FALSE", {
  ...
  res_tes = names(tmp) == 'token'  && length(tmp$token) == 5 && is.vector(tmp$token) && class(tmp) == "tokenization and transformation" && substring(tmp$token[4], 1, 4) == '_at_'
